...
| カテゴリー | 内容の例 |
|---|---|
| クイズ（10） | ピンポン／ざんねん／ジャカジャン／ドラムロール／合格／早おしボタン |
...
| ゲーム（13） | ジャンプ／コイン／パワーアップ／１UP／土管／ステージクリア／たからばこ |
| 和（7） | ポクポク（木魚）／小鼓／和太鼓／さんじょう／ひょうしぎ／ほら貝 |
| ネタ（11） | ネコ／イヌ／トラ／カラス／ウグイス／カメラ／ブー |
...
- **効果音：[Freesound](https://freesound.org)**（すべてCC0 / パブリックドメイン）
  ...
  - わらい声：[female laughter by aliceemily](https://freesound.org/people/aliceemily/sounds/257487/)
...
- 「ゴング（れんだ）」は「ゴング」と音の高さが同じで区別がつかなかったため，5半音上へ移調しています
- 「ゆうれい」は「ひゅ～」（風）と「どろどろ」（ティンパニのロール）の2素材をつないで作っています
- 「ジャンプ」「コイン」「パワーアップ」「１UP」「土管」の5音は，このアプリ用に合成した
  チップチューンです（素材配布元・ゲーム会社とは無関係の自作音です）
...
- 共有URLの先頭にはバージョン番号を持たせ，音の入れ替えで番号の意味が変わったときは
  古いURLを読み込まない設計です（誤って別の音が鳴るのを防ぐため）。現在はバージョン5
- CSS・JS・MP3の読み込みには `?v=6` を付けています。音を差し替えたときは
